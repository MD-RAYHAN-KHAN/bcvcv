...
def project():
    ...
    #buttons
            ...
            self.btnTeacher = Button(ButtonFrame, text="Teacher", font=('times new roman', 20, 'bold'), height=1, width=10, bd=4, command=Teacher)
            self.btnTeacher.grid(row=1, column=2)

            self.btnStudent = Button(ButtonFrame, text="Student", font=('times new roman', 20, 'bold'), height=1, width=10, bd=4, command=Student)
            self.btnStudent.grid(row=1, column=3)
...
def splashscreen():
    ...
    t.speed(10)
    t.hideturtle()
    t.write("LEARN TO SR IT", move=True, align="center", font=("Roboto Bold", 50, "bold"))#,"underline"
    t.goto(-250, 0)
    t.penup()
    t.goto(-250, 10)
    t.bk(70)
